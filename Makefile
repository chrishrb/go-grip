...
# Build flags
LDFLAGS=-s -w
VERSION_FLAGS=-X github.com/chrishrb/go-grip/cmd.Version=$(VERSION) -X github.com/chrishrb/go-grip/cmd.CommitHash=$(COMMIT) -X github.com/chrishrb/go-grip/cmd.BuildDate=$(BUILD_DATE)
...
release: clean ## Create a new release (make sure to git tag first)
	@echo "Creating release $(VERSION)"
	@if echo $(VERSION) | grep -q "dirty"; then \
		echo "Error: Working directory is dirty. Commit changes before making a release."; \
		exit 1; \
	fi
	mkdir -p bin/release
